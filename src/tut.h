...
namespace tut
{
  ...
  struct group_base
  {
    ...
    // Finalization for group.
    virtual void group_completed() = 0;
  ...
  namespace 
  {
    ...
    /**
     * Tests first object to be less than than second.
     * Throws if false.
     *
     * NB: both T and Q must have operator << defined somewhere, or
     * client code will not compile at all!
     */
    template <class T,class Q>
    void ensure_lessthan(const char* msg,const Q& lesser,const T& greater)
    {
      if ( ! (lesser<greater) )
      {
        std::stringstream ss;
        ss << (msg?msg:"") << (msg?": ":"") << "expected " << lesser
           << " < " << greater;
        throw failure(ss.str().c_str());
      }
    }
    ...
    template <class T,class Q>
    void ensure_lessthan(const Q& lesser,const T& greater)
    {
      ensure_lessthan<>(0,lesser,greater);
    }

    /**
     * Tests first object to be greater than second.
     * Throws if false.
     *
     * NB: both T and Q must have operator << defined somewhere, or
     * client code will not compile at all!
     */
    template <class T,class Q>
    void ensure_greaterthan(const char* msg,const Q& greater,const T& lesser)
    {
      if ( ! (greater>lesser) )
      {
        std::stringstream ss;
        ss << (msg?msg:"") << (msg?": ":"") << "expected " << greater
           << " > " << lesser;
        throw failure(ss.str().c_str());
      }
    }
    ...
    template <class T,class Q>
    void ensure_greaterthan(const Q& greater,const T& lesser)
    {
      ensure_greaterthan<>(0,greater,lesser);
    }
  ...
  class test_group : public group_base
  {
    ...
    typedef void (*groupfinalization)();

    tests tests_;
    tests_iterator current_test_;
    groupfinalization group_finalization_;
     ...
     */
    test_group(const char* name, groupfinalization final = NULL)
      : name_(name), group_finalization_(final)
    {
      // register itself
    ...
    test_group(const char* name, test_runner& another_runner)
      : name_(name), group_finalization_(NULL)
    {
      // register itself
    ...
    void group_completed() {
      if (group_finalization_ != NULL) group_finalization_();
    }
